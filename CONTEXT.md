...
**Weapon rarity**:
A catalog tier on every **Weapon**: **Common**, **Uncommon**, **Rare**, **Epic**, **Legendary**. Ten catalog keys with exactly two **Weapons** per rarity (and still two per **Weapon type**). Catalog **Weapon** nudges are authored at **Common-scale** bases; at runtime `resolveWeaponNudges(key)` multiplies every nudge field by a linear **Weapon rarity strength** (Common **1.0**, Uncommon **1.25**, Rare **1.5**, Epic **1.75**, Legendary **2.0** — Legendary is **2×** Common). Combat (`effectiveStats`), max **Life** (`maxLifeForSeat`), and **Weapon** UI summaries all use scaled nudges. **Weapon** offer sampling is **weighted by rarity** (integer weights per tier: Common **40**, Uncommon **25**, Rare **15**, Epic **10**, Legendary **5** — both keys of a tier share the same weight); still **3 distinct** weapons per seat via weighted sample **without replacement**; **Legendary** rarest. **Weapon pick** offer cards signal rarity with a colored card border (not a written chip on the card); rarity remains in the offer button `aria-label`. Display colors (muted kylix palette): **Common** gray (`#9aa3ad`), **Uncommon** green (`#72b072`), **Rare** blue (`#6a9ec8`), **Epic** purple (`#a082c8`), **Legendary** orange (`#d8924a`).
_Avoid_: Shrinking or expanding the ten-key catalog for rarity; rarity-gated **Draft** omit rules; uniform **Weapon** offer sampling; per-weapon bespoke nudge formulas outside Common-scale × strength; loud neon rarity chrome.
...
- **Weapon catalog (v1):** ten keys in `src/match/weaponCatalog.ts` (two per type, two per rarity). Light nudges only (still floor 500ms after all); catalog nudges are **Common-scale** bases scaled by **Weapon rarity strength** at runtime.
- **Weapon rarity (v1):** **Common**, **Uncommon**, **Rare**, **Epic**, **Legendary** — every catalog **Weapon** has exactly one; two keys per tier. **Strength multipliers:** Common **1.0**, Uncommon **1.25**, Rare **1.5**, Epic **1.75**, Legendary **2.0** (Legendary **2×** Common); `resolveWeaponNudges(key)` applies to combat, max **Life**, and UI. Offer sampling is **weighted by rarity** (Common **40**, Uncommon **25**, Rare **15**, Epic **10**, Legendary **5**; both keys per tier share weight); **3 distinct** per seat; **Legendary** rarest. **Weapon pick** uses colored offer borders (not a written chip on the card). Display colors: Common gray, Uncommon green, Rare blue, Epic purple, Legendary orange (hex values in **Weapon rarity** lock above).
...
- **Neutral damage primary-fire (v1):** each damage **God**'s any-weapon neutral primary-fire has lower raw DPS than that **God**'s two **Required Weapon type** damage primary-fires — **Draft** accessibility tradeoff (mismatched **Weapon** seats can still pick damage, but weapon-aligned seats get stronger fire).
...
- **Catalog authorship (v1):** do **not** author a **Passive** as `own` + **God** filter + **potency** up (flat or percent). Same-**God** fire density in a 5-slot **Loadout** is too low for that payoff. Prefer same-**God** **Cooldown**, kind-wide **Cooldown**/potency, **Weapon type** gates, or enemy-facing **Cooldown**/potency. Existing **God**-filtered potency rows are being reworked under that rule; engine filters unchanged.
